...
class RegistrationController extends AbstractActionController
{
	...
	public function indexAction()
	{
        ...
        if ($request->isPost()) {
            ...
            $fileArr = $this->params()->fromFiles('image_file');
			...
			$form->setData($post);
			 if ($form->isValid()) {
                 ...
                 $size = new \Zend\Validator\File\Size(array('min' => 1,'max'=> 4194304 )); // minimum bytes filesize, max too..
                 ...
                 if($adapter->isValid()) {
                     ...
                     $adapter->setDestination('./public/img/profile/');
                     $adapter->addFilter('File\Rename',
                         array('target' => $adapter->getDestination() . "/{$uid}_profile.png",
                             'overwrite' => true));

                     if ($adapter->receive($fileArr['name'])) {
                         $filename = $adapter->getFilter('File\Rename')->getFile();
                         $profile_pic = $filename[0]['target'];
                     }


                     $data = $form->getData();
                     $data['raw_password'] = $form->get('usr_password')->getValue();
                     ...
                     $this->flashMessenger()->addMessage($auth->usr_name);
                     return $this->redirect()->toRoute('auth/default', array('controller' => 'admin', 'action' => 'list'));
                 }
                 else if(empty($fileArr['name']))
                 {
                     $data = $form->getData();
                     $data['raw_password'] = $form->get('usr_password')->getValue();
                     $data = $this->prepareData($data);
                     ...
                     $this->flashMessenger()->addMessage('Error en el archivo de imagen.');
                 }
			}			 
		}
    ...
    public function editAction()
    {
        ...
        if (!$id) {
            return $this->redirect()->toRoute('auth/default',
                array('controller'=>'admin', 'action'=>'list'));
        }

        $userDM = $this->getUsersTable()->getUser($id);
        ...
        $form->get('usr_password')->setValue($userDM->raw_password);
        ...
        if ($request->isPost()) {
            ...
            $userDM->usr_password_salt = $this->generateDynamicSalt();
            $userDM->usr_password = $this->encriptPassword(
                $this->getStaticSalt(),
                $form->get('usr_password')->getValue(),
                $userDM->usr_password_salt
            );
            $userDM->raw_password = $form->get('usr_password')->getValue();

            $date = new \DateTime();
            $userDM->lng_id = 1;
            $userDM->usr_registration_date = $date->format('Y-m-d H:i:s');
            $userDM->usr_registration_token = md5(uniqid(mt_rand(), true));

            $userDMC = clone $userDM;

            if ($form->isValid()) {

                $form->getData()->usr_password_salt = $userDMC->usr_password_salt;
                $form->getData()->usr_password = $userDMC->usr_password;
                $form->getData()->usr_registration_date = $userDMC->usr_registration_date;
                $form->getData()->lng_id = $userDMC->lng_id;
                $form->getData()->usr_registration_token = $userDMC->usr_registration_token;
                $form->getData()->raw_password = $userDMC->raw_password;
...
//					$file_old = "{$uid}_profile.png";
                    ...
                    $userDMC->photo = $profile_pic;
                    $form->getData()->photo = $userDMC->photo;
        ...
        return new ViewModel(array('form' => $form, 'usr_id' => $userDM->usr_id, 'user' => $userDM));
    }
